[README_RC10.md](https://github.com/user-attachments/files/31683202/README_RC10.md)
...
La guía de programas japonesa sigue mostrando sus menús en japonés. No es un problema de instalación: esa interfaz se genera desde el script compilado `epg.amx`, que sobrescribe las etiquetas definidas en los recursos de texto. RC10 no modifica ejecutables ni scripts compilados por seguridad y compatibilidad. Los nombres de canales y programas, además, proceden de los datos EPG japoneses originales.
...
The Japanese Programme Guide still displays its menus in Japanese. This is not an installation problem: the interface is generated by the compiled `epg.amx` script, which overwrites labels defined in text resources. RC10 does not modify executables or compiled scripts for safety and compatibility. Channel and programme names also originate from the original Japanese EPG data.
